...
class RagItemHelpers:
    ...
    def document_exists(self, source: str) -> bool:
        try:
            results = self.vector_store.similarity_search(source, k=1)
            return any(source in doc.metadata.get("source", "") for doc in results)
        except Exception as e:
            print(f"Error checking existence for {source}: {e}")
            return False

    async def upload_web_content_to_chroma(self, urls):
        print("Uploading web content to Chroma...")
        for url in urls:
            if not self.document_exists(url):
    ...
    async def upload_pdfs_to_chroma(self, pdf_files):
        ...
        for pdf_file in pdf_files:
            ...
            if self.document_exists(pdf_file):
                print(f"PDF {pdf_file} already exists in the database.")
                continue
